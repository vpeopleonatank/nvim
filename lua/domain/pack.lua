local vim_path = require('domain.global').vim_path
...
local pack = setmetatable({}, { __index = { repos = {} } })
  ...
  for file in p:lines()do
    ...
    for repo,conf in pairs(repos) do
      if conf == true then
        ...
        local tmp = vim.tbl_extend('force',{repo},conf)
        table.insert(self.repos,tmp)
      end
    end
  end
end
...
function pack:load_repos()
  ...
  if vim.fn.has('vim_starting') then
    if not fs.isdir(packer_dir) then
      os.execute(cmd)
